...
export interface IImageAddonOptions {
  ...
  iipSupport: boolean;
  iipSizeLimit: number;
  // Kitty Unicode-placeholder inline images (redraw-safe). Image bytes are fetched
  // out-of-band from `imageEndpoint` + <id>; ids arrive encoded in a placeholder
  // cell's truecolor fg, (row,col) in its combining diacritics.
  kittyPlaceholders: boolean;
  imageEndpoint: string;
  kittyForceDiacriticWidth: boolean;
}
...
// A Kitty-placeholder image: sliced into an explicit gridCols×gridRows tile grid
// (the cell block the emitter laid out), so tile mapping is exact and rounding-proof
// regardless of pixel size (ImageRenderer.draw prefers gridCols over ceil(w/cellW)).
